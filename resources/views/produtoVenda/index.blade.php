	...
	<div id="example_wrapper" class="dataTables_wrapper dt-bootstrap5 ">
        <div class="row w-100">
            ...
            <div class="col-sm-12 col-md-6"></div>
                <div id="example_filter" class="dataTables_filter">
                    <a href="{{ route('produtoVenda.create') }}" class="btn btn-success float-right">Adicionar</a>
                    </div>
                </div>
            </div>
        ...
        <thead>
			<tr role="row">
                ...
                <th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Age: activate to sort column ascending">Categoria</th>
                <th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Salary: activate to sort column ascending" >Cor</th>
                <th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Salary: activate to sort column ascending">NCM</th>
                <th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Salary: activate to sort column ascending">OBS</th>
		...
		<tbody>
    		...
    		@foreach ($produtosVenda as $produto)
                <tr role="row" class="odd">
                    ...
                    <td class="sorting_1">PRO - {{ $produto->product_sale_id }}</td>
                    <td>{{ $produto->product_sale_name }}</td>
                    <td>{{ $produto->product_sale_category }}</td>
                    <td>{{ $produto->product_sale_color }}</td>
                    <td>{{ $produto->product_sale_ncm }}</td>
                    <td>{{ $produto->product_sale_obs }}</td>
                    <td>R$ {{ number_format($produto->product_sale_price,2,",",".") }}</td>
                    <td>
                        <form action="{{ route('produtoVenda.destroy',['produtoVenda'=> $produto->product_sale_id]) }}" method="post">
                        @csrf
                        @method('delete')
                            <a href="{{ route('produtoVenda.edit',['id'=> $produto->product_sale_id]) }}" class="btn btn-warning">Editar</a>
